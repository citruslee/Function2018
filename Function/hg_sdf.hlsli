...
float BoxCheap(float3 p, float3 b) { //cheap box
	return vmax(abs(p) - b);
}
...
float Box2Cheap(float2 p, float2 b) {
	return vmax(abs(p) - b);
}
...
//   float box0 = Box(p, float3(1));
//   float box1 = Box(p-float3(1), float3(1));
